...
impl Checkpoint {
    /// Returns the number of partitions affected by the checkpoint.
...
impl CheckpointDelta {
    ...
    /// Returns the number of partitions affected by the checkpoint delta.
    pub fn len(&self) -> usize {
        self.per_partition.len()
    }
}
...
mod tests {
    ...
    #[test]
    fn test_add_checkpoint_delta() -> anyhow::Result<()> {
        let mut delta1 = {
            let mut delta = CheckpointDelta::default();
            delta.add_partition(
                PartitionId::from("a"),
