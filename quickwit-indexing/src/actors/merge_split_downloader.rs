...
impl AsyncActor for MergeSplitDownloader {
    async fn process_message(
        ...
        let merge_scratch_directory = self
            .create_scratch_and_download_splits(&merge_operation.splits)
            .await?;
...
impl MergeSplitDownloader {
    pub async fn create_scratch_and_download_splits(
        &self,
        splits: &[SplitMetadata],
    ) -> anyhow::Result<ScratchDirectory> {
        ...
        for split in splits {
            ...
            let dest_path = merge_scratch_directory.path().join(split_file);
            self.storage.copy_to_file(split_file, &dest_path).await?;
        }
        Ok(merge_scratch_directory)
    }
}
...
mod tests {
    ...
    async fn test_merge_split_downloader() -> anyhow::Result<()> {
        ...
        let (merge_split_downloader_mailbox, merge_split_downloader_handler) =
            universe.spawn_async_actor(merge_split_downloader);
